...
function    createStudent(studentData) {
    ...
    return getAssetRegistry('mx.itesm.gradeexchanger.courses.Course')
        .then(function(studentRegistry){
            ...
            var   relationship = factory.newRelationship('mx.itesm.gradeexchanger.career','Career',studentCourseData.careerId);
            studentsSt.career = studentData.careerId;
            // 6. Emit the event FlightCreated
            var event = factory.newEvent(NS, 'CourseCreated');
            event.courseId = courseId;
            emit(event);

            return studentRegistry.addAll([studentsSt]);
        });
}
