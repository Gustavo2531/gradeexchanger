 ...
 * 
 * **/
function    createStudent(careerData) {
    // 1. Get the asset registry
    return getAssetRegistry('mx.itesm.gradeexchanger.career.Career')
        .then(function(careerRegistry){
            ...
            var  careerSt = factory.newResource(NS,'Career',careerData.careerId);
           
            // 4. Set the relationship
            
  
            careerSt.careerName = careerData.name;
            careerSt.units = careerData.units;
            ...
            event.careerId = careerSt.careerId;
            emit(event);

            return studentRegistry.addAll([careerSt]);
        });
}
